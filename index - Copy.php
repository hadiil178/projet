...
try {
    ...
    $sql = "SELECT *, original_price, discounted_price, discount_percentage FROM produits WHERE 1";
    ...
    $categories_stmt = $conn->query("SELECT DISTINCT nom FROM categories ORDER BY nom");
    $all_categories = $categories_stmt->fetchAll(PDO::FETCH_COLUMN);

    // --- Dynamic Banner Data from Products ---
    // Fetch a few product images for the banner (you can adjust the LIMIT)
    $banner_stmt = $conn->query("SELECT id, nom, image_path FROM produits ORDER BY RAND() LIMIT 3");
    $dynamic_banners = $banner_stmt->fetchAll(PDO::FETCH_ASSOC);
    // ---------------------------------------
...
<head>
    ...
    <style>
        ...
        .sidebar-links a i {
            margin-right: 10px;
            color: #8C5356; /* Accent color for icons */
        }

        .sidebar-links a.active-sidebar-link {
            background-color: #F5CACB; /* Lighter accent for active link */
            color: #8C5356;
            font-weight: bold;
        }
        ...
        .main-content {
            flex-grow: 1;
            padding-top: 70px; /* Adjusted padding to accommodate header */
            width: 100%;
        ...
        .dynamic-banner-slide img {
            ...
            max-height: 550px; /* Match the container's max-height */
        }
        ...
        .search-bar-container {
            ...
            margin-top: 12px; /* Adjust margin to space from banner */
            margin-left: auto;
            margin-right: auto;
            position: relative; /* Ensure proper stacking */
            z-index: 1; /* Keep it below the header if it overlaps */
        }
        ...
        .price {color: #8C5356; /* Darker accent for price */
            font-style: italic;
            font-size: 0.9em; /* Smaller font size */
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Dynamic Banner Auto-Slide (Basic Implementation using product images)
            const bannerSlides = document.querySelectorAll('.dynamic-banner-slide');
            let currentSlide = 0;
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-links">
            <a href="./edit_profile.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'edit_profile.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-user"></i> Edit Profile</a>
            <a href="./add_product.php?category=perfume" class="<?php if (strpos($_SERVER['PHP_SELF'], 'add_product.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-plus"></i> Add Product</a>
            <a href="./manage_products.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'manage_products.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-cubes"></i> Manage Products</a>
            <a href="./ajoutercategory.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'ajoutercategory.html') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-folder-plus"></i> Add Category</a>
            <a href="./manage_categories.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'manage_categories.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-folder-open"></i> Manage Categories</a>
            <a href="./Statistics.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'Statistics.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-chart-bar"></i> View Statistics</a>
            <a href="./orders.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'orders.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-shopping-basket"></i> Orders</a>
            <a href="./customer.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'custumer.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-users"></i> Customers</a>
            <a href="./add_admin.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'add_admin.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-user-plus"></i> Add admin</a>
            <a href="./apply_promotion.php" class="<?php if (strpos($_SERVER['PHP_SELF'], 'apply_promotion.php') !== false) echo 'active-sidebar-link'; ?>"><i class="fas fa-tag"></i> Apply Promotion</a>
        </div>
    </div>

    <div class="main-content">
        <div class="dynamic-banner-container">
            <?php if (isset($dynamic_banners) && is_array($dynamic_banners) && count($dynamic_banners) > 0): ?>
                <?php foreach ($dynamic_banners as $banner): ?>
                    <div class="dynamic-banner-slide">
                        <a href="product_details.php?id=<?php echo htmlspecialchars($banner['id']); ?>">
                            <img src="<?php echo htmlspecialchars($banner['image_path']); ?>" alt="<?php echo htmlspecialchars($banner['nom']); ?>">
                        </a>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="dynamic-banner-slide active">
                    <img src="uploads/default_banner.jpg" alt="Default Banner">
                </div>
            <?php endif; ?>
        </div>
        ...
        <?php elseif (!empty($products)): ?>
            <div class="product-category">
                <?php
                $category_title = 'Our Exquisite Products';
                if ($active_category !== 'all') {
                            ...
                            <div class="product-item">
                                <a href="product_details.php?id=<?php echo $product['id']; ?>">
                                    ...
                                    <?php if (isset($product['discount_percentage']) && $product['discount_percentage'] > 0): ?>
                                        <div class="original-price" style="text-decoration: line-through; color: #777; font-size: 0.8em;"><?php echo htmlspecialchars($product['original_price']); ?> DA</div>
                                        <div class="discounted-price price"><?php echo htmlspecialchars($product['discounted_price']); ?> DA</div>
                                        <div class="discount-percentage" style="color: green; font-size: 0.8em;">(<?php echo htmlspecialchars(round($product['discount_percentage'])); ?>% Off)</div>
                                    <?php else: ?>
